...
CREATE POLICY admin_bypass_policy ON private.channels FOR ALL TO app_admin USING (true);
CREATE POLICY admin_bypass_policy ON private.channel_members FOR ALL TO app_admin USING (true);
CREATE POLICY admin_bypass_policy ON private.messages FOR ALL TO app_admin USING (true);
...
CREATE POLICY insert_channel_by_user ON private.channels FOR INSERT WITH CHECK (user_id = private.current_user_id());
...
RETURNS TRIGGER AS $$
BEGIN
  -- Insert a row into the channel_members table
...
-- add member to channel on first message
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    -- If the user is not a member, add them to the channel_members table
